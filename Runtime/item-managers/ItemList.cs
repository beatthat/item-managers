        ...
        where ItemType : class
    {
        ...
        public IEnumerable<ItemType> GetAll()
        {
            foreach (var listItem in m_listItems)
            {
                ...
                var asItem = v as ItemType;
                if (asItem != null)
                {
                    yield return asItem;
                    continue;
                }
                var hasItems = v as IHasItems;
                if (hasItems == null)
                {
                    throw new InvalidCastException(
                        "the ListItemType of a ScrollList must either be the same as the ItemType or it must implement IHasItems ItemType="
                        + typeof(ItemType).Name + " ListItemType=" + typeof(ListItemType).Name);
                }
                foreach (var i in hasItems.GetItems<ItemType>())
                {
                    yield return i;
                }
            }
        ...
        {
            using (var items = ListPool<T>.Get())
            {
                GetItems<T>(items);
